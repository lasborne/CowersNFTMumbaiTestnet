...
describe('NFT Contract', () => {
    ...
    beforeEach(async() => {
        ...
        const NftContract = await ethers.getContractFactory('CowersNFT', deployer)
    ...
    describe('runs the NFT smart contract', () => {
        ...
        it ('Checks the ability to mint all tokens even if some mints have been done', async() => {

            let valueMint = ethers.utils.parseEther('0.005')
            await nftContract.connect(user1).functions.mint('0x', 5, {value: valueMint})
            await nftContract.connect(user2).functions.mint('0x', 2, {value: valueMint})
            await nftContract.connect(user3).functions.mint('0x', 11, {value: valueMint})
            
            let totalSupply = await nftContract.totalSupply()
            console.log(totalSupply)

            await nftContract.functions.mintAll('0x', 18)
            let allMints = await nftContract.allMintsShow()
            console.log(await allMints)
            console.log(await allMints.length)
            expect(allMints.length).to.equal(21)
        })
    })
})