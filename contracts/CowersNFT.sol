...
string constant name = "Cowers";
string constant symbol = "CWR";

contract CowersNFT is ERC721(name, symbol), ReentrancyGuard{
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, 'Must be Cowers NFT deployer');
        _;
    }
    ...
    notOwner incorrectAmount nonReentrant returns (bool) {
        ...
        require (totalSupply < maxSupply, "Maximum of 21 NFTs available to mint!");
    ...
    public payable returns (AllMints[] memory) {
        ...
        require((total + totalSupply) <= maxSupply, "Maximum of 21 NFTs available to mint!");
        uint256 existingMintsCount;
        for (uint256 j = 1; j < (total + 1); j++) {
            if(_exists(j)) {
                existingMintsCount++;
            }
        }
        AllMints[] memory allMints = new AllMints[](total + 1 + existingMintsCount);
        
        for (uint256 i = 1; i < (total + 1 + existingMintsCount); i++) {
